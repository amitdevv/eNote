...
3. **One accent + one bounded palette.** Green (`#16A34A`) is reserved for identity (workspace avatar, pin, selected state, focus rings). Labels/statuses/priorities use the separate 12-hue palette in §2.6 — never the brand green. Buttons and rows stay neutral.
...
| `--brand` | `#16A34A` | Avatar, active identity (rationed) |
